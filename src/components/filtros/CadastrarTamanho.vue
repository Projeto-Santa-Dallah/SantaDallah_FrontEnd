...
const props = defineProps({
 ...
 idEditar: { type: Number, default: null } // se vier, é edição
});
...
// Refs
const openAddTamanho = ref(props.open);
...
// Carregar categorias e, se necessário, dados para edição
onMounted(async () => {
 await useCategorias.getCategorias();
 tamanho1.value = await tamanhosApi.buscarPorId(props.idEditar) // aqui você passa o ID que quer
  console.log(tamanho1.value.nome)
 

 if (props.idEditar) {
   // Buscar tamanho existente na store ou via API
   const existente = await tamanhosApi.buscarPorId(props.idEditar);
   if (existente) {
     Object.assign(tamanho, existente);
     categoriaSelecionada.value = existente.categoria?.id;
   } else {
     const resposta = await useTamanho.getTamanhoById(props.idEditar);
     Object.assign(tamanho, resposta);
     categoriaSelecionada.value = resposta.categoria?.id;
   }
 }
});
...
watch(() => props.open, (novo) => {
 openAddTamanho.value = novo;
 if (!novo && !props.idEditar) {
   // Limpar form só no cadastro
...
// Função de salvar (cadastro ou edição)
async function salvar() {
 try {
   ...
   if (props.idEditar) {
     // Edição
     await useTamanho.salvarTamanho({ ...tamanho });
   } else {
     // Cadastro
     await useTamanho.salvarTamanho({ ...tamanho });
   }
   ...
   // Atualizar lista na store
   await useTamanho.getTamanhos();
 } catch (e) {
   console.error("Erro ao salvar tamanho:", e);
 }
}
</script>
...
<template>
   ...
   <div class="tamanhos-header">
     <div class="header">
       <h1 class="titulo-tamanhos">
         {{ props.idEditar ? "Editar Tamanho" : "Cadastrar Tamanho" }}
       </h1>
     </div>
   </div>
     ...
     <form @submit.prevent="salvar">
       ...
       <div class="buttons-container">
         ...
         <button class="button" type="submit">
           {{ props.idEditar ? "Salvar Alterações" : "Adicionar Tamanho" }}
         </button>
       </div>
     </form>
   </div>
 ...
 <div v-if="confirmacao" class="confirmacao">
   <div class="container">
     ...
     <span>Tamanho {{ props.idEditar ? "atualizado" : "cadastrado" }} com sucesso!</span>
