...
function getProductById(id) {
  return carrinho.produtos?.find?.(p => p.id === id)
}
...
/* --- Computed para tamanho ---  
1 item → exibe o primeiro (ex: "P")
2+ itens → "X camadas"
*/
const tamanhoDisplay = computed(() => {
  if (!props.tamanho || props.tamanho.length === 0) return 'P'
  if (props.tamanho.length === 1) return props.tamanho[0]
  return `${props.tamanho.length} camadas`
})

/* --- Computed para nome e sabor customizado --- */
const nomeCompleto = computed(() => {
  if (props.sabor?.toLowerCase() === 'personalizado') {
    return `${props.nome} – Massa e recheio personalizados`
  }

  return `${props.nome} – ${props.sabor}`
})
    ...
    <div class="item">
      ...
      <div class="item-info">
        ...
        <div class="item-quantidade">
          <!-- Ícone de lixeira quando quantidade = 1 -->
          <svg
            v-if="quantidade < 2"
          ...
          <!-- Botão normal quando quantidade > 1 -->
